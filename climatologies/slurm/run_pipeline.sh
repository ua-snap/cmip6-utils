#!/bin/bash
# Build the job list, (re)generate the sbatch scripts from config.yaml, and
# submit the fragments array job followed by the combine job (chained with
# --dependency=afterok so combine only runs once every fragment succeeds).
set -euo pipefail
