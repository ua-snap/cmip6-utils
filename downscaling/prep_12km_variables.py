...
def rename_variable_in_file(
    ...
    """
    Rename a variable in a NetCDF file, optionally convert C to K, and save to output location.
    ...
    conversion_note = " (C -> K)" if convert_to_kelvin else ""
    print(f"  Processed: {input_file.name} -> {output_file.name}{conversion_note}")
...
def process_variable(
    ...
    conversion_note = " (C -> K)" if convert_to_kelvin else ""
    print(f"\nProcessing variable: {old_var} -> {new_var}{conversion_note}")
    ...
    for nc_file in nc_files:
        ...
        try:
            rename_variable_in_file(
                nc_file, output_file, old_var, new_var, convert_to_kelvin=convert_to_kelvin
            )
