...
* `prep_12km_variables.py` - This script is used to prepare daily 12km ERA5 outputs by changing the file variable names, file names, and directory names to match the 4km dataset. It also supports converting temperature variables from Celsius to Kelvin (use `--celsius-to-kelvin`) to match the ERA5 data units expected by the downscaling flow.
* `create_12km_target_grid_file.py` - This script is used to subset a time slice from the 12km ERA5 `tasmax` to use as a target grid file.
